...
const SOURCE_POLICY: &str = r#"Use direct primary sources whenever possible. The discovery triad is
required, not interchangeable: use Hermes web_search for Exa semantic discovery;
also query $AI_NEWS_SEARXNG_SEARCH_URL through terminal/curl for broad meta-search;
use Browse.sh local browser automation and installed site skills for dynamic,
search-gated, or extraction-resistant candidates. A basic SearXNG request uses
curl -sG "$AI_NEWS_SEARXNG_SEARCH_URL" --data-urlencode format=json
--data-urlencode "q=<query>". Choose categories, engines, language, time_range,
and pageno for the question; inspect unresponsive_engines and retry weak results
with source-qualified queries, broader categories, or a looser time range. Use
